import React from 'react';
import type { CategoryType } from '../types/category';

interface CategoryItemProps {
  category: CategoryType;
  onEdit: (category: CategoryType) => void;
  onDelete: (id: number) => void;
  isUpdating?: boolean;
  isDeleting?: boolean;
}

const CategoryItem: React.FC<CategoryItemProps> = ({ category, onEdit, onDelete, isUpdating = false, isDeleting = false }) => {
  return (
    <div className="flex w-full py-4 px-12 bg-white rounded-[30px] text-lg items-center">
      <div className="w-[40%] text-gray-700">{category.name}</div>
      <div className="w-[40%] text-gray-600">{category.nameUzb}</div>
      <div className="w-[20%] flex items-center justify-start gap-3">
        <button 
          className={`text-blue-500 hover:text-blue-700 ${isUpdating ? 'opacity-50 cursor-not-allowed' : ''}`}
          onClick={() => !isUpdating && onEdit(category)}
          disabled={isUpdating}
          title={isUpdating ? "Обновление..." : "Редактировать"}
        >
          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
            <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" />
          </svg>
        </button>
        <button 
          className={`text-red-500 hover:text-red-700 ${isDeleting ? 'opacity-50 cursor-not-allowed' : ''}`}
          onClick={() => !isDeleting && onDelete(category.id)}
          disabled={isDeleting}
          title={isDeleting ? "Удаление..." : "Удалить"}
        >
          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
            <path fillRule="evenodd" d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM7 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v6a1 1 0 102 0V8a1 1 0 00-1-1z" clipRule="evenodd" />
          </svg>
        </button>
      </div>
    </div>
  );
};

export default CategoryItem;
